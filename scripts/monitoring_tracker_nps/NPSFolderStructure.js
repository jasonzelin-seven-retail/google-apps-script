/*******************************
 * NPS FOLDER MANAGER - OPTIMIZED VERSION
 * Mengatasi Execution Time Limit
...
// OPTIMIZED: Increased batch size, reduced safety margin
const BATCH_SIZE = 50;  // Disesuaikan untuk stabilitas
const MAX_EXECUTION_TIME = 330000;  // 5.5 menit
const LIST_BATCH_LIMIT = 500;  // Disesuaikan
const DAILY_UPDATE_HOUR = 12;

// NEW: Cache Service untuk folder lookups
const CACHE_DURATION = 21600;  // 6 jam

// CENTER MAPPING (Pastikan ini sudah didefinisikan)
const CENTER_FOLDER_MAP = {
  'BSD': 'BSD',
  'TJD': 'Tangerang',
  'BGR': 'Bogor'
  // Tambahkan center lainnya sesuai kebutuhan
};
...
/*******************************
 * OPTIMIZED: FILE COUNT + LINKS - FAST VERSION
 *******************************/
function getFilesDataFromSubfolder(parentFolder, subfolderName) {
  try {
    ...
    const targetFolder = filesFolder.next();
...
/*******************************
 * UPDATE FILE COUNTS - BATCH VERSION (OPTIMIZED)
 *******************************/
...
function continueUpdateFileCounts() {
  ...
  try {
    ...
    while (lastProcessedRow < totalRows) {
      // Check timeout - sisakan 60 detik untuk safety
      if (new Date().getTime() - startTime > (MAX_EXECUTION_TIME - 60000)) {
      ...
      // Hanya baca kolom D (Folder URL)
      const data = destSheet.getRange(startRow, 4, numRows, 1).getValues();
      const updateBuffer = [];
      
      for (let i = 0; i < data.length; i++) {
        const folderUrl = data[i][0]; // Column D: Folder URL
      ...
      // Write batch to sheet (Kolom E-H: PC Count, PC Links, GC Count, GC Links)
...
function showCreateFoldersDialog() {
  const html = HtmlService.createHtmlOutput(`
    ...
    <div class="tip">
      ...
      Per batch: ${BATCH_SIZE} rows (50 rows untuk stabilitas).<br><br>
      Struktur: Center / Year-Week / SA Name - Class ID / {Group Chat, Personal Chat} / Files<br>
      ⚡ Optimized untuk kecepatan tanpa timeout!
    </div>
...
/*******************************
 * CREATE FOLDERS - CORE AUTO BATCH (OPTIMIZED)
 *******************************/
...
function createNestedFolderStructure(parentFolder, centerCode, year, week, saNameClassCode) {
  ...
  getOrCreateFolder(groupChatFolder, 'Files');
  getOrCreateFolder(personalChatFolder, 'Files');
